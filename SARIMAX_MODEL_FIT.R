...
x_train <- as.matrix(train_data[, ..selected_xreg])
...
if (!is.null(sarimax_css)) {
  cat("CSS estimation successful — proceeding to ML refinement.\n")
  sarimax_fit <- tryCatch(
    Arima(y_train_ts,
          order = ORDER,
          seasonal = list(order = SEASONAL, period = PERIOD),
          xreg = x_train_scaled,
          method = "CSS-ML",
          fixed = coef(sarimax_css)),
    error = function(e) {
...
if (is.null(sarimax_fit)) {
  cat("Final fallback: CSS-only estimation.\n")
  sarimax_fit <- Arima(y_train_ts,
                       order = ORDER,
                       seasonal = list(order = SEASONAL, period = PERIOD),
                       xreg = x_train_scaled,
                       method = "CSS")
