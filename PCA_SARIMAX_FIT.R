# ===============================================================
# PCA-BASED SARIMA MODEL FIT — TRAIN DATA ONLY (NO FORECASTING)
# ===============================================================
...
# --- TRAINING DATA ---
train_data <- model_data[as.Date(interval) < "2024-10-01"]
...
if (!is.null(sarima_pca_css)) {
  cat("CSS estimation successful — proceeding to ML refinement.\n")
  sarima_pca_fit <- tryCatch(
    Arima(y_train_ts,
          order = ORDER,
          seasonal = list(order = SEASONAL, period = PERIOD),
          xreg = x_train_scaled,
          method = "CSS-ML",
          fixed = sarima_pca_css$coef),
    error = function(e) {
...
# --- Summary ---
if (!is.null(sarima_pca_fit)) {
  cat(sprintf("\nAIC: %.2f | BIC: %.2f | LogLik: %.2f\n",
              sarima_pca_fit$aic, sarima_pca_fit$bic, sarima_pca_fit$loglik))
  print(summary(sarima_pca_fit))
  
  # --- Residual Diagnostics ---
  residuals_fit <- residuals(sarima_pca_fit)
  cat("\n--- Residual Diagnostics ---\n")
  lb_test <- Box.test(residuals_fit, lag = 24, type = "Ljung-Box", 
                      fitdf = length(sarima_pca_fit$coef))
  print(lb_test)
