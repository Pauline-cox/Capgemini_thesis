...
create_env_clusters <- function(train_data, n_clusters = 3) {
  ...
  list(clusters = km$cluster, centers = km$centers, env_vars = available_env, n_clusters = n_clusters)
}

assign_env_clusters <- function(test_data, train_data, cluster_obj) {
  X_train <- scale(as.matrix(train_data[, ..cluster_obj$env_vars]))
  X_test <- scale(as.matrix(test_data[, ..cluster_obj$env_vars]),
                  center = attr(X_train, "scaled:center"),
                  scale = attr(X_train, "scaled:scale"))
  apply(X_test, 1, function(x) which.min(colSums((t(cluster_obj$centers) - x)^2)))
}
...
create_cluster_features <- function(train_data, test_data, cluster_obj) {
  ...
  list(train = train_dt[, ..feature_vars], test = test_dt[, ..feature_vars], feature_names = feature_vars)
}
...
rolling_sarimax_cluster_24h <- function(train_data, test_data, cluster_features, order, seasonal, period) {
  ...
  model <- Arima(ts(y_train, frequency = period),
                 order = order,
  ...
  for (h in seq(-22, n_test - 23)) {
    ...
    updated <- tryCatch(Arima(ts(hist_y, frequency = period), model = model, xreg = hist_x),
  ...
  list(forecasts = forecasts, runtime = total_time, train_time = train_time, predict_time = predict_time)
}
...
run_sarimax_cluster <- function(train, test, label) {
  ...
  eval[, `:=`(Runtime_min = res$runtime, Train_min = res$train_time, Predict_min = res$predict_time, Period = label)]
...
saveRDS(
  list(
    model = "SARIMAX_Cluster_24h",
    period_A = resultsA_sarimax_cluster,
    period_B = resultsB_sarimax_cluster,
    evaluations = all_eval_sarimax_cluster,
    parameters = list(order = ORDER, seasonal = SEASONAL, period = PERIOD, clusters = N_CLUSTERS)
  ),
  file = save_name
)
cat(sprintf("\nResults saved to: %s\n", save_name))
