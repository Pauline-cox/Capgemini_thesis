# ============================================================
# HYBRID MODEL: Pretrained SARIMAX + LSTM Residual 24h Forecast
...
plot_forecast <- function(dt, model_name, label, color = "darkgreen") {
  ggplot(dt, aes(x = Time)) +
    ...
    labs(title = paste(model_name, "- 24h Ahead Forecast -", label),
         x = "Time (hours)", y = "Energy Consumption (kWh)", color = "Series") +
    theme_minimal(base_size = 12)
}

# ------------------------------------------------------------
# Residual extraction & preprocessing
# ------------------------------------------------------------
...
train_lstm_residual <- function(train_data, test_data, feature_columns) {
  ...
  # --- convergence plot
  df <- data.frame(epoch = seq_along(history$metrics$loss),
  ...
  p_conv <- ggplot(df, aes(x = epoch)) +
    ...
    labs(title = "LSTM Residual Training Convergence",
         x = "Epoch", y = "Loss (MAE)", color = "Legend") +
  ...
  list(forecasts = preds, model = model,
       runtime = runtime, train_time = train_time,
       convergence_plot = p_conv)
}

# ------------------------------------------------------------
# Main hybrid forecast wrapper
# ------------------------------------------------------------
run_hybrid_residual_24h <- function(train, test, sarimax_period_obj, y_train_ts, label) {
  cat("\n>>> Starting Hybrid (SARIMAX + LSTM Residual 24h Forecast) -", label, "<<<\n")
  ...
  total_runtime <- as.numeric(difftime(Sys.time(), overall_start, units = "mins"))
...
save_name <- sprintf("Results_Hybrid_SARIMAX_LSTM_FullResiduals_%s.rds", timestamp)
saveRDS(list(
  model = "Hybrid_SARIMAX_LSTM_Residual24h_Full",
  period_A = resultsA_hybrid,
  period_B = resultsB_hybrid,
  evaluations = all_eval_hybrid,
  parameters = list(lookback = LOOKBACK, horizon = HORIZON,
